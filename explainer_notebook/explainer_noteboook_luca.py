...
# %%
it = (i for i in graph_wiki.nodes(data=True))
t = next(it)
...
def color_func_wiki(word, **kwargs):
    colors = [
        "#bcbcbc",
        "#030303",
        "#6b6b6b",
...
for node in tqdm(graph_reddit.nodes):
    wc = lf.wordcloud_from_node(graph_reddit, node, color_func=color_func_reddit)
    wc.to_file(
        Config.Path.shared_data_folder
        / "wordclouds"
        / "reddit"
        / f"{node.replace('/','_')}.png"
    )

# %%
for node in tqdm(graph_wiki.nodes):
    print("generating wordcloud for node:" + node)
    wc = lf.wordcloud_from_node(graph_wiki, node, color_func=color_func_wiki)
    print("generated wordcloud for node:" + node)
    ...
    print("saved wordcloud for node:" + node)


# %%
wc = lf.wordcloud_from_link(graph_reddit, "caffeine", "theanine")
